...
SEED_NUMBER                      = 0
USE_CUDA                         = False # When doing quantisation set to False as only works with CPU at the moment.
...
COMPRESSION_TECHNIQUE            = "PTQ"      # Option: "PTQ" "QAT"

MODEL_DIR                        = "../../models/" + MODEL_CHOICE

MODEL_TO_BE_COMPRESSED_FILENAME  = MODEL_VARIANT +"_"+DATASET_NAME+".pt"
MODEL_TO_BE_COMPRESSED_FILEPATH  = os.path.join(MODEL_DIR, MODEL_TO_BE_COMPRESSED_FILENAME)

COMPRESSED_MODEL_FILENAME        = COMPRESSION_TECHNIQUE+"_"+MODEL_VARIANT +"_"+DATASET_NAME+".pt"
...
def main():
    ...
    # Compress model
    compressed_model = compress_model(model, COMPRESSION_TECHNIQUE, device, train_loader)
    ...
    _,compressed_model_eval_accuracy     = top1Accuracy(model=compressed_model, test_loader=test_loader, device=device, criterion=None)
